...
class Depurar(threading.Thread):
    ...
    def procesar_ternario(self, instruccion, ts):
        ...
        id = instruccion.id
        condicion = instruccion.condicion
        op1 = instruccion.op1
        op2 = instruccion.op2
        #es una declaracion
        if instruccion.tipo:
            ...
            if not ts.existe(id):
                ...
                ts.add(new_simbol)
                condicionante = self.procesar_operacion(condicion,ts)
                new_if = self.generarIF()
                new_cuadruplo = Cuadruplo("if",condicionante,"goto",new_if)
                self.cuadruplos.add(new_cuadruplo)
                self.etiquetas[self.etiqueta].append(new_cuadruplo)
                #aqui ponemos si es falso
                resultado_falso = self.procesar_operacion(op2,ts)
                new_cuadruplo = Cuadruplo("=",resultado_falso,"",temp)
                self.cuadruplos.add(new_cuadruplo)
                self.etiquetas[self.etiqueta].append(new_cuadruplo)
                #agregamos goto para la continuacion de las sentnecias
                new_cuadruplo = Cuadruplo("goto",new_end,"","")
                self.cuadruplos.add(new_cuadruplo)
                self.etiquetas[self.etiqueta].append(new_cuadruplo)
                #hacemos la etiqueta principal el if
                self.etiqueta = new_if
                self.etiquetas[self.etiqueta] = []
                #agregamos las sentencias a si es verdadero 
                resultado_verdadero = self.procesar_operacion(op1,ts)
                new_cuadruplo = Cuadruplo("=",resultado_verdadero,"",temp)
                self.cuadruplos.add(new_cuadruplo)
                self.etiquetas[self.etiqueta].append(new_cuadruplo)
                #agregamos etiqueta para volver
                new_cuadruplo = Cuadruplo("goto",new_end,"","")
                self.cuadruplos.add(new_cuadruplo)
                self.etiquetas[self.etiqueta].append(new_cuadruplo)
